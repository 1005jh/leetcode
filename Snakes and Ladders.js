...
var snakesAndLadders = function (board) {
  ...
  const n2 = n * n;
  const visited = new Array(n2 + 1).fill(false);
  visited[1] = true;
  ...
  while (queue.length > 0) {
    ...
    for (let i = 1; i <= 6; i++) {
      let nextPos = pos + i;
      if (nextPos > n2) break;
      const [row, col] = getBoardPosition(nextPos, n);
      if (board[row][col] !== -1) {
...
function getBoardPosition(s, n) {
  let row = Math.floor((s - 1) / n); // s 위치의 행 인덱스
  let col = (s - 1) % n; // s 위치의 열 인덱스

  if (row % 2 === 1) {
    col = n - 1 - col;
  } // 행이 홀수 번째인 경우에는 열이 반대 방향으로 배치되어 있으므로 열 인덱스를 뒤집어준다.

  row = n - 1 - row; // 행도 아래서부터 시작하므로 뒤집어준다.
