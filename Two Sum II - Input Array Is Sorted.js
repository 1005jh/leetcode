...
var twoSum = function (numbers, target) {
  let result = [];
  findIndex(numbers, 0, target, result);
  return result;
};
function findIndex(arr, left, target, result) {
  let right = left + 1;
  if (right === arr.length) return;
  else if (0 < target && target < arr[left]) return;
  else if (target <= 0 && 0 < arr[left]) return;
  ...
  left++;
  findIndex(arr, left, target, result);
}
